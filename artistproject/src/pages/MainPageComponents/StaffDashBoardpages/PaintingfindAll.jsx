...
export default function PaintingfindAll() {
  ...
  const getPaintingList = async () => {
    const api = path + "/PTController/selectall";
    // 等同 $.ajax(" get blablablba ")
    try {
      const result = await axiosInstance.get(`${api}`);
      // console.log(result);
      setPaintingData(result.data)
    ...
    return paintingData.map((a, i) => {
      ...
      return (
        <tr key={i}>
          ...
          <td><img className="img-thumbnail"src={`data:image/jpeg;base64,${a.image}`}  alt="Photo"/></td>
